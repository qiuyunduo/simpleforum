package com.qyd.core.autoconf.property;
...
public class PlaceholderHelper {
    ...
    public Set<String> extractPlaceholderKeys(String propertyString) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (endIndex + PLACEHOLDER_SUFFIX.length() < strVal.length() - 1) {
                String remainPart = normalizeToPlaceholder(strVal.substring(endIndex + PLACEHOLDER_SUFFIX.length()));
                if (!StringUtils.isEmpty(remainPart)) {
                    stack.push(remainPart);
                }
            }
        }
    ...
    public boolean isNormalizedPlaceholder(String propertyString) {
        return propertyString.startsWith(PLACEHOLDER_PREFIX) && propertyString.endsWith(PLACEHOLDER_SUFFIX);
    }

    public boolean isExpressionWithPlaceholder(String propertyString) {
    ...
    private int findPlaceholderEndIndex(CharSequence buf, int startIndex) {
        ...
        int withinNextedPlaceholder = 0;
        while (index < buf.length()) {
            if (StringUtils.substringMatch(buf, index, PLACEHOLDER_SUFFIX)) {
                if (withinNextedPlaceholder > 0) {
                    withinNextedPlaceholder--;
