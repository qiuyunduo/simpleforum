...
/**
 * 配置变更注册，找到@Value 注解修饰的配置，
 * 注册到SpringValueRegistry, 实现统一的配置变更自动刷新管理
 *
 * @author 邱运铎
 * @date 2024-05-13 14:34
 */
@Slf4j
@Component
public class SpringValueProcessor implements BeanPostProcessor {
    ...
    protected void processMethod(Object bean, String beanName, Method method) {
        // registry @Value on method
        Value value = method.getAnnotation(Value.class);
        if (value == null) {
            return;
        }
        // Skip Configuration bean mthod
        ...
        for (String key : keys) {
            SpringValueRegistry.SpringValue springValue = new SpringValueRegistry.SpringValue(
                    key, value.value(), bean, beanName, method);
            SpringValueRegistry.register(key, springValue);
            log.debug("Monitoring {}", springValue);
        }
    }
