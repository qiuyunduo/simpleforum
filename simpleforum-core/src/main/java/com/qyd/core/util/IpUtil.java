...
import com.qyd.core.region.IpRegionInfo;
...
public class IpUtil {
    ...
    public IpUtil() throws SocketException {
    }
    ...
    private static List<Inet4Address> getLocalIp4AddressFromNetWorkInterface() throws SocketException {
        // 这里list大小设置1，应该是认为一般完整正常服务器进仅存在一个网卡信息
        ...
        while (networkInterfaces.hasMoreElements()) {
            ...
            // 过滤回环网卡，点对点网卡，非活动网，虚拟网卡，并要求网卡名称是eth或者ens开头
     ...
     *
     * @param ni    网卡
     * @return      如果满足要求则返回true, 否则返回false
     */
    ...
    /**
     * 通过Socket 唯一确定一个IP
     * 当有多个网卡的时候，使用这种方式一把可以得到想要的IP, 甚至不要求外网地址8.8.8.8是可连通的
     *
     * @return Inet4Address
     */
    ...
    public static String getClientIp(HttpServletRequest request) {
        try {
            // X-Real-IP：通常只在反向代理服务器中使用,通常是使用了nginx代理才会产生该字段
            String xIp = request.getHeader("X-Real-IP");
            // X-Forwarded-For：由于它可以记录整个代理链中的IP地址，每当请求经过一个代理服务器时，该代理服务器会将其IP地址追加到X-Forwarded-For字段的末尾
             ...
             * HTTP_X_FORWARDED_FOR这几个参数获取到的IP地址都有可能是真实的IP地址，
             * 但是它们的可信度和准确性是不同的。如果要获取最接近真实的客户端IP地址，
             * 建议优先使用X-Forwarded-For、Proxy-Client-IP、HTTP_CLIENT_IP、
             * HTTP_X_FORWARDED_FOR这几个参数。
             */
    ...
    public static IpRegionInfo getLocationByIp(String ip) {
        // 使用全局的vIndex 创建带 VectorIndex 缓存的查询对象
        initVIndex();
