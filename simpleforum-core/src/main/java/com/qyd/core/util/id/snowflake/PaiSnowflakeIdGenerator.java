 ...
 * 自定义实现的雪花算法生成器
 *
 * id结构： 时间 + 数据中心(3位) + 机器id(7位) + 序列号（12位）
 *
 * @author 邱运铎
 * @date 2024-05-13 19:24
 */
@Slf4j
public class PaiSnowflakeIdGenerator implements IdGenerator{
    ...
    public synchronized Long nextId() {
        long nowTime = waitUntilNextTime(getNowTime());
        if (lastTime == nowTime) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("second: {}, datacenter: {}, work:{}, seq:{}, ans:{}", nowTime % DateUtil.ONE_DAY_SECONDS,
                    dataCenter, workId, sequence, ans);
        }
     ...
     * 就个人感觉该方法功能和下面waitUntilNextTime功能是一样的
     *
     * @param nowTime  当前时间戳
     * @return   返回新的时间戳
     */
