...
public class SkyWalkingTraceIdGenerator {
    ...
    private static class IDContext {
        ...
        private IDContext(long lastShiftTimestamp, short threadSeq) {
            this.lastShiftTimestamp = lastShiftTimestamp;
            this.threadSeq = threadSeq;
        }

        private long nextSeq() {
            return timestamp() * 10_000 + nextThreadSeq();
        }

        private long timestamp() {
            long currentTimeMillis = System.currentTimeMillis();
            // Just for considering time-shift-back by Ops or OS. @hanahmily 's suggestion
            if (currentTimeMillis < lastShiftTimestamp) {
