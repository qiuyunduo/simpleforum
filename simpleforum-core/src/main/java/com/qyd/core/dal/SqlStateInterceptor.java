...
public class SqlStateInterceptor implements Interceptor {
    ...
    private String buildSql(StatementHandler statementHandler) {
        ...
        if (statementHandler.getParameterHandler() instanceof DefaultParameterHandler) {
            ...
            configuration = ((MappedStatement) ReflectionUtils.getFieldVal(parameterHandler, "mappedStatement", false)).getConfiguration();
         }
     ...
     * 生成要执行的sql命令
     * 我的理解是为了将
     * mybatis中的原始的 sql: select * from table where id = ? and name = ?
     * 转换为真实的sql: select * from Table where id = 1 and name= "tom"
     *
     * @param boundSql
     * @param configuration
     * @return
    ...
    private String getSql(BoundSql boundSql, Configuration configuration) {
        ...
        for (ParameterMapping parameterMapping : parameterMappings) {
            ...
            // quoteReplacement 转义字符串中的特殊字符，将其当普通字符串处理
