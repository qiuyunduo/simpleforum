package com.qyd.web.config.init;
...
public class ForumDataSourceInitializer {
    ...
    public DataSourceInitializer dataSourceInitializer(final DataSource dataSource) {
        ...
        boolean needInit = needInit(dataSource);
        initializer.setEnabled(needInit);
        initializer.setDatabasePopulator(databasePopulator(needInit));
        return initializer;
    }

    private DatabasePopulator databasePopulator(boolean initEnable) {
        final ResourceDatabasePopulator populator = new ResourceDatabasePopulator();
        // 下面这种事根据sql文件来进行初始化； 改成liquibase 之后不再使用这种方案， 由liquibase来统一管理表结构数据变更
        if (initEnable && !liquibaseEnable) {
            // fixme: 首次启动时。对于不支持liquibase的数据。 如mariadb,采用主动初始化
    ...
    private boolean needInit(DataSource dataSource) {
        ...
        if (!liquibaseEnable) {
            // 非liquibase做数据库版本管理的，根据是否存在用户表来判断是否初始化
        ...
        // 对于liquibase做版本管控的场景，若使用的不是默认的数据库，则需要进行修订
        // 初始数据库中的liquibase中的版本ID是00000000000020
        ...
        if (Objects.equals(record.get(0).get("MD5SUM"), "8:a1a2d9943b746acf58476ae612c292fc")) {
            ...
            // MD5SUM='8:bb81b67a5219be64eff22e2929fed540' 是 记录版本ID是00000000000020对应的值，这里保证数据是作者正确提供的
    ...
    private boolean autoInitDatabase() {
        ...
        URI uri = URI.create(SpringUtil.getConfigOrElse("spring.datasource.url", "spring.dynamic.datasource.master.url").substring(5));
        ...
        try (Connection connection = DriverManager.getConnection("jdbc:mysql://" + uri.getHost() + ":" + uri.getPort() +
