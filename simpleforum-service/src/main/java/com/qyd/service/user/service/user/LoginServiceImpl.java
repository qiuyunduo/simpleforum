...
import com.qyd.service.user.service.help.StarNumberHelper;
...
public class LoginServiceImpl implements LoginService {
    ...
    public Long autoRegisterWxUserInfo(String uuid) {
        UserSaveReq req = new UserSaveReq()
                .setLoginType(LoginTypeEnum.WECHAT.getType())
                .setThirdAccountId(uuid);
    ...
    public String loginByUserPwd(String username, String password) {
        ...
        // 1. 为了兼容历史数据，对于首次登陆成功的用户，初始化ai信息
    ...
    public String registerByUserPwd(UserPwdLoginReq loginReq) {
        ...
        if (user != null) {
            if (!userPwdEncoder.match(user.getPassword(),  loginReq.getPassword())) {
                // 3.1 用户名已经存在， 走不到下面的注册流程，且密码错误，代表不是该用户本人
    ...
    private void registerPreCheck(UserPwdLoginReq loginReq) {
        ...
        if (StringUtils.isNotBlank(starNumber)) {
            if (Boolean.FALSE.equals(starNumberHelper.checkStarNumber(starNumber))) {
                // 星球编号校验不通过，直接抛异常
                throw ExceptionUtil.of(StatusEnum.USER_STAR_NOT_EXISTS);
            }
