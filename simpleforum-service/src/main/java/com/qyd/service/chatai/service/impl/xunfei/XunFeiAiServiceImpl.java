...
import com.qyd.api.model.enums.WsConnectStateEnum;
...
import com.qyd.service.chatai.service.impl.chatgpt.ChatRecordWxVo;
import lombok.Data;
...
public class XunFeiAiServiceImpl extends AbsChatService {
    ...
    public AiChatStatEnum doAsyncAnswer(Long user, ChatRecordsVo response, BiConsumer<AiChatStatEnum, ChatRecordsVo> consumer) {
        XunFeiChatWrapper chat = new XunFeiChatWrapper(String.valueOf(user), response, consumer);
     ...
     * 比较麻烦的是讯飞建立连接60s没有返回主动断开，问了一次返回结果之后也主动断开， 下次需要重新建立连接
     */
    @Data
    ...
    public class XunFeiMsgListener extends WebSocketListener {
        ...
        public void onMessage(@NotNull WebSocket webSocket, @NotNull String text) {
            ...
            if (responseData.successReturn()) {
                ...
                pl.getChoices().getText().forEach(t -> {
                    msg.append(t.getContent());
                });
                item.appendAnswer(msg.toString());

                if (responseData.firstResponse()) {
        ...
        public void onClosed(@NotNull WebSocket webSocket, int code, @NotNull String reason) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("连接终端! code={}, reason={}", code, reason);
            }
